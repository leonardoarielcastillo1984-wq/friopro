...
# Run seed
print_step "Poblando base de datos con datos iniciales..."
npm run seed:complete > "$LOG_DIR/seed.log" 2>&1 || true
...
# ═══════════════════════════════════════════════════════════════════════════
# STEP 6: START MOCK API SERVER (para frontend)
# ═══════════════════════════════════════════════════════════════════════════

print_header " STEP 6: Iniciando Mock API (puerto 3001)..."

cd "$APPS_WEB"
node --experimental-modules mock-api-server.js > "$LOG_DIR/mock-api.log" 2>&1 &
MOCK_PID=$!
echo $MOCK_PID > "$LAUNCHER_DIR/mock-api.pid"
print_step "Mock API iniciada (PID: $MOCK_PID)"

# Wait for Mock API to be ready
print_step "Esperando que Mock API esté listo..."
for i in {1..30}; do
  if curl -s http://localhost:3001/auth/csrf > /dev/null 2>&1; then
    print_success "Mock API está lista en http://localhost:3001"
    break
  fi
  if [ $i -eq 30 ]; then
    print_warning "Timeout esperando Mock API (continuando de todas formas)"
    break
  fi
  echo -n "."
  sleep 1
...
print_header " STEP 7: Iniciando servidor API (puerto 3002)..."
...
print_header " STEP 8: Iniciando servidor Web (puerto 3000)..."
...
print_header " STEP 9: Abriendo navegador..."
...
echo -e "   ${GREEN}✓${NC} Mock API: Corriendo en puerto 3001"
...
echo -e "   ${CYAN}Mock API:${NC} http://localhost:3001"
