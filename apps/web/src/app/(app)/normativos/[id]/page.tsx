...
import { Sparkles, Lightbulb, FileCheck, AlertCircle, CheckCircle, X } from 'lucide-react';
...
export default function NormativoDetailPage() {
  ...
  async function loadClauseDetail(clauseId: string) {
    ...
    try {
      const res = await apiFetch<{ clause: NormativeClause & { content: string; childClauses: NormativeClause[] } }>(
        `/normativos/${id}/clauses/${clauseId}`,
      );
      setSelectedClause(res.clause);
    } catch (err: any) {
      ...
      <div>
        ...
        >
          ...
          {clause._count?.documentMappings ? (
            <span className="ml-auto mr-3 rounded bg-blue-100 px-1.5 py-0.5 text-xs text-blue-700">
