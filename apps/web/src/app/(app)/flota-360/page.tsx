...
import { AlertTriangle, Plus, QrCode, ChevronRight, Truck, ScanLine, CalendarClock, Wrench } from 'lucide-react';
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">Operación de mantenimiento de flota en un solo lugar</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-3">
        <div className="space-y-2">
          <div className="flex items-center gap-0.5 border-b border-neutral-200 bg-white rounded-t-lg px-1">
            {[
                ...
                {lista.map((o) => {
                  ...
                  return (
                    <tr key={o.id} className="hover:bg-neutral-50">
                      ...
                      <td className="px-2.5 py-2 text-neutral-700">{o.vehiculo?.dominio || o.activoNombre || '—'}</td>
                      <td className="px-2.5 py-2">
                      ...
                      <td className={`px-2.5 py-2 flex items-center gap-1 ${meta.color}`}>
                        <OrigenIcon className="h-3 w-3 shrink-0" /> <span className="text-[11px]">{o.origen}</span>
                      </td>
                    </tr>
                  );
                })}
              ...
              <ul className="space-y-1">
                {data.inmovilizados.map((v) => (
                  <li key={v.id} className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className="text-[11px] text-neutral-500 truncate">{v.notas || 'En taller'}</span>
                  </li>
                ))}
              </ul>
            )}
