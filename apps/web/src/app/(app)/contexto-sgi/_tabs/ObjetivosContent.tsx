...
export default function ObjetivosContent() {
  ...
  return (
    <GenericCrudPage
      ...
      aiFields={[
        { targetKey: 'description', buttonLabel: 'Desarrollar objetivo', buildPrompt: (f) => `Eres un consultor ISO. Objetivo:\nTítulo: ${f.title || '—'}, Norma: ${f.standard || '—'}, Año: ${f.year || '—'}\n\nRedactá una descripción completa del objetivo. Máximo 2 párrafos.` },
        { targetKey: 'notes', buttonLabel: 'Indicadores sugeridos', buildPrompt: (f) => `Para el objetivo:\nTítulo: ${f.title || '—'}, Meta: ${f.target || '—'}\n\nSugerí 3 indicadores con nombre, fórmula, frecuencia y fuente. Formato numerado.` },
      ]}
    />
  );
}
