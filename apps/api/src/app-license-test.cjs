...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // POST /license/subscription (crear suscripción)
    if (url === '/license/subscription' && method === 'POST') {
      if (!auth?.userId) {
        res.writeHead(401, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Unauthorized' }));
        return;
      ...
      const body = await parseBody(req);
      const { planTier, period, provider = 'manual' } = body;
      ...
      const subscription = {
        ...
        endDate: new Date(Date.now() + (period === 'annual' ? 365 : 30) * 24 * 60 * 60 * 1000).toISOString(),
        autoRenew: true
      };
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 API SGI 360 License Test funcionando en puerto ${PORT}`);
