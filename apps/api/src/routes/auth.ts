...
export const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  // ── POST /forgot-password — Envía enlace de reset por email ──
  app.post('/forgot-password', strictRateLimit, async (req: FastifyRequest, reply: FastifyReply) => {
    const schema = z.object({ email: z.string().email() });
    ...
    // El token usa el hash actual como parte del secret — se invalida al cambiar la contraseña
    const resetSecret = jwtSecret + user.passwordHash.slice(-16);
    const resetToken = jwt.sign({ userId: user.id, email: user.email }, resetSecret, { expiresIn: '30m' });

    const appUrl = process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || 'https://logismart.ar';
  ...
  // ── POST /reset-password — Aplica la nueva contraseña con el token ──
  app.post('/reset-password', strictRateLimit, async (req: FastifyRequest, reply: FastifyReply) => {
    const schema = z.object({
      token: z.string().min(1),
      password: z.string().min(8, 'La contraseña debe tener al menos 8 caracteres'),
    ...
    try {
      ...
      return reply.code(400).send({ error: e.errors?.[0]?.message || 'Datos inválidos' });
    }
    ...
    if (!decoded?.userId) return reply.code(400).send({ error: 'Token inválido' });
    ...
    const resetSecret = jwtSecret + user.passwordHash.slice(-16);
    try {
      jwt.verify(body.token, resetSecret);
    } catch {
      return reply.code(400).send({ error: 'El enlace de recuperación expiró o ya fue utilizado. Solicitá uno nuevo.' });
    }
